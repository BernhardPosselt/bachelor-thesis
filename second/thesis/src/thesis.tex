...
\put(-32,-150){
\begin{minipage}{14cm}
\bfseries\huge Softwaretest von Web-Applikationen
\end{minipage}
}
...
Andere Umgebung als normale Applikationen: testen von datenbanken, web servern und clientside verschiedene browser, alles über netzwerk, nicht eine technologie sondern verschiedene (JavaScript, Flash, HTML, CSS, Serverside sprache, SQL)
\cite{testing_apps_on_web}[Foreword]

Server client model früher nur terminal server die zum anzeigen von server response genutzt wurden \cite{testing_apps_on_web}[S. 13] heute mehr logik auf dem client (JavaScript)
Event handling is komplexer weil verschiedene events in unterschiedlicher reihenfolge auftreten können \cite{testing_apps_on_web}[S. 18]
Mehrere laufende Instanzen sind komplex wg. sync \cite{testing_apps_on_web}[S. 20]

Web UI implementation
System integration
server + client installation
configuration + compability
database
security
performance/load/stress
\cite{testing_apps_on_web}[S. 29]
